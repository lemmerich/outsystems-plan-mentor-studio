...
description: >
  ...
  not about what happens inside one action. Adapted from upstream for the
  Mentor Studio (chat, no MCP) channel — see DELTA.md.
---
...
2. **No data-model leakage into the agent.** If the agent's own
   ...
   an `AnalisarItemComIA` action that takes an `ChecklistItem` record
   directly instead of generic text), the agent becomes unreusable for
   a second checklist type, a second app, or a second tenant. The agent's
...
- Verify the boundary actually holds: publish a change to the agent
  alone and confirm the calling app's own module history in ODC Studio
  shows no new revision. If it does, something leaked across the
  boundary.
...
testing its `AgentFlow`/service action directly (ODC Studio's own test
console, or a temporary debug screen/button per `prototype-to-widgets.md`
#25's "Dev tools" pattern — there is no MCP harness in this channel) with
a real input, and inspecting its raw output — before writing or debugging
...
**How to apply:**
- Before wiring a new/changed agent into any calling app action, test its
  own service action (or `AgentFlow` directly, if accessible) via ODC
  Studio's test console or a temporary debug screen, with a real,
  representative input. Read the raw output verbatim — don't paraphrase
  or assume its shape from the spec.
...
**Verify:** the agent's isolated raw output, read directly from that
standalone test — not the calling app's summary of what it did with that
...
for those. Add a new entry here when a wave-scoping decision (not a bug,
not a widget fix) turns out to matter enough that a future wave should
make the same call automatically instead of re-deriving it.
