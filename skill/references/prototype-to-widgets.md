...
## 28. A "duplicate this record" action (Clone/Duplicate) can echo copied fields in the client without ever persisting them server-side

An action that creates a new record from an existing one (a "Duplicate" button producing a new draft version pre-filled with the source's Name/Code/Description) can appear to work immediately after the fix that adds the copy — the new form renders with the right values, and even `element.value` read via JS confirms it — while the values were never actually written into the new record before its `CreateOrUpdate`/commit, only assigned to the on-screen input's client state as a post-creation echo. The tell is specific: the bug is invisible as long as verification stays on the same page load (client state still holds the copied value), and only appears after navigating away and back, forcing a fresh fetch from the server, at which point the fields come back empty. A DOM read of `.value` immediately after the action fires is not proof of persistence — it proves the client rendered *something*, which can come from either the server response or from the action's own optimistic client-side assignment. **Verify any "duplicate/copy fields" fix by navigating away from the screen and back (or reloading) before trusting a DOM value check** — the same discipline as `backend-and-data-gotchas.md` #10's "a successful fetch response proves nothing about what reacts to it," applied to create-by-copy actions specifically.

## 29. A CSS fix can be typographically present but visually invisible — read computed `color` against computed `background`, not just presence of a value
...
## 30. Editing CSS on the platform's own responsive layout shell (sidebar/header spacing) can silently break the built-in mobile/tablet breakpoint behavior — test both a wide AND a narrow viewport after any such change
...
## 34. Two CSS rules that each look individually correct can still leave an exact-pixel dead zone where neither one's condition is true
...
## 35. A repeated request to fix a placeholder string can get "fixed" by rewording the placeholder instead of implementing the real rendering behind it

When visible output is a static/hardcoded string standing in for dynamic content that was never actually wired up (e.g. `"(see options in edit mode)"` where real option pills should render), a fix prompt that describes the *symptom* ("show the real options, not this message") can get satisfied by simply editing the string's wording to sound more accurate, without adding the actual dynamic rendering — because from Mentor's perspective, changing what the text says is a valid, complete response to "this text is wrong." This can repeat multiple times, each turn producing a differently-worded but still-static placeholder, none of which is progress. **Break the loop by describing the required DOM shape, not the current text's wrongness**: state explicitly how many elements should exist and what each should contain (e.g. "the container must end up with N child elements, one per option, each with the option's text — not a single text node"), and reference the exact source of the real data that a sibling field already binds correctly (e.g. "read the same value already powering the `OptionsInput` field"). Verify the fix by checking the element's *child count*, not just that the visible text changed.

## 36. A row/card's Edit/Save/Cancel buttons rendering doesn't prove an edit mode was ever implemented for it — count actual form fields, not buttons

Two structurally similar repeated-list patterns in the same app (e.g. item cards and classification-range table rows, both needing inline add/edit/remove) can be built at different times with very different completeness — one fully working (real `<input>`s appear on Edit, values save), the other rendering all four action buttons (Edit/Save/Cancel/Remove) permanently and doing nothing when any of them (other than a simple client-side Remove) is clicked, because the row's cells were never given an edit-mode template at all — every cell is always a static `<span>`, with zero `<input>`/`<select>`/`<textarea>` anywhere in the row, in any state. This is easy to under-diagnose as "the button handler is broken" when the real gap is "there is no form to switch to." **Verify by querying the row's actual form-control count** (`row.querySelectorAll('input, select, textarea').length`) both before and after clicking "Edit" — if it's zero in both states, no edit mode exists yet, and the fix needs to build one (ideally copying the pattern from whichever sibling list in the same app already has it working), not just re-wire a button's click handler.

## 37. `data-test` on a repeated list must be verified on the individual repeated item, not just found "somewhere" in the container
...
## 38. A dropdown can show a selected option while its bound variable still holds `EmptyValue`

ODC dropdowns distinguish "an option is visually current" from "the user actually fired the selection interaction." A user who opens the dropdown and clicks away without click-selecting a specific option can leave the bound variable at its configured `EmptyValue` (e.g. `NullIdentifier()`) even though the closed control shows a real label. Code that trusts the variable then operates on an empty/default Id — which, combined with `backend-and-data-gotchas.md` #15, usually surfaces as "record not found for Id=0" rather than as anything that looks dropdown-shaped. **When a dropdown feeds an action that only makes sense with a real selection, ask for an explicit fallback for the case where the bound variable still equals `EmptyValue` when the action fires** (e.g. default to the first item of the list that populated it) — never assume a rendered option implies an active selection. Low-stakes "pick one, then act" widgets (a Dev tools picker, a quick filter) are where this is most likely to be skipped.

## 39. Asking Mentor to "add" a visibility/enabled condition to an element that already has one replaces the old condition instead of combining with it

Mentor implements "hide these buttons when X" on buttons that were already hidden when `IsReadOnly` by writing X as the element's new, sole `Visible`/`Enabled` expression — not by ANDing X onto what was already there. The new condition works exactly as asked, the old one is gone, and the element is now interactive in a state that used to lock it down. This is invisible in the fix's own target state and only surfaces in a *different* state the prompt never mentioned: a fix that branched a checklist item's answer buttons on `Weight = 0` silently un-did the same buttons' existing "hidden when the record is Finalized" rule for every item — caught only because an earlier wave's automated test re-ran the read-only case. **When a prompt adds a condition to an element that already has one, name the existing condition and state what the two must do together** ("keep it hidden when read-only regardless of X; split on X only when not read-only"). After any such fix, re-run the state that was already correct before it, not just the new condition's own target case — ideally through the existing test for that state.

## 40. A native `<select>`'s open option list ignores the page's dark theme

Most browsers render a plain `<select>`'s open options popup with browser/OS-native colors, ignoring the page's own dark-theme CSS — only the closed control is reliably restyleable. Option text colored for a dark card (a light gray) then lands on the popup's native white background: technically styled, practically illegible. **Set an explicit dark text color on `<option>` wherever the app uses a plain select on a dark theme.** Two things are effectively uncontrollable across browsers and not worth a further fix round once confirmed: the popup's own background color and the highlight color of the currently-selected row — genuine platform ceilings, not an unfixed prompt.

## 41. A dark theme needs `color-scheme: dark`, or native browser chrome stays light

A dark-themed app still showing a light native scrollbar (and light checkboxes, date pickers, other form-control chrome) never set the CSS `color-scheme` property — the browser has no signal to render its own UI in dark style and defaults to light regardless of how dark the page's content is. **Set `color-scheme: dark` in the dark-mode variable block (and `color-scheme: light` in the light default, or `light dark` to let the browser pick) on `:root`/`html`.** One declaration fixes native chrome across the whole app; a hand-rolled `::-webkit-scrollbar` rule set (Chromium-only, separate track/thumb/hover rules per theme) is unnecessary for this.

## 42. Resize the side menu through `--side-menu-size`, never with a per-selector `width` override

The side menu's own container (`.app-menu-content`) is `position: fixed` in OutSystems UI, so a percentage width on it (`width: 100%`, present in the framework's default CSS) resolves against the *viewport*, not its wrapper. Overriding the wrapper's width with `width: Npx !important` leaves the inner fixed element untouched — the menu then vanishes or paints over the entire page. **Override the one custom property the framework already keys off — `--side-menu-size` — once, at `:root` or `.layout-side` scope** (check its current value and every rule that sets it first, including `.aside-visible`/`.aside-expandable` variants). Every framework rule that positions or sizes the menu, and the content area's `margin-left`, already reads it, so one override cascades everywhere a hand-written per-selector fix would have to be rediscovered piece by piece. Combine with #30: re-verify at a narrow width afterward.

## 43. An icon weight class (`ph-light`, `ph-bold`) renders nothing unless that weight's font file is actually loaded

An icon library typically ships one weight's font file at a time (e.g. Phosphor Regular, class `ph`); only that weight has matching `font-family` and glyph `::before` content rules. Asking for `ph-light ph-<name>` produces a class that matches nothing — no font, no glyph — and the icon renders empty, not visibly thinner. **Before asking for a weight variant, confirm that weight's stylesheet is actually loaded in the app** (a resource named `Phosphor-Light`/`-Bold`, not just the base one). If it isn't, add it first or stay on the loaded weight — a class name alone never changes which font file renders.

## 44. One unclosed CSS rule silently swallows every rule after it, so a bulk CSS change "applies partly"

Browsers now support CSS nesting, so a rule missing its closing brace no longer raises a parse error that discards the rest — **every subsequent rule is absorbed into it as a nested child** (`.pill::before { & .opt-btn {…} & .toggle-pill {…} }`), where it matches nothing. The rules are present in the stylesheet, read correctly, and never apply; it presents exactly like "Mentor ignored the tail of my request" or "it undid a fix we already had," which sends the investigation after a behavioral cause for a syntax bug. **After any bulk CSS change, check that the LAST rule of the block exists as a top-level rule**: `[...document.styleSheets].flatMap(s=>{try{return [...s.cssRules]}catch(e){return []}}).filter(r=>r.selectorText==='<last-selector>')`. If it's missing, or an earlier rule's `cssText` contains `&` children nobody wrote, that earlier rule is the unclosed one — fix from there down. Prefer several short CSS changes over one long one: the failure is silent and its blast radius is everything below the mistake.

## 45. Compare against a desktop prototype at a real desktop width — OutSystems UI's tablet/phone rules below ~1024px override what you're verifying

OutSystems UI ships responsive rules (`.tablet .btn`, `.phone .btn`, the mobile menu collapse) that fire below roughly 1024px and override desktop rules with higher specificity. A browser pane or headless viewport at its default width (often ~800px) shows a missing sidebar, different button sizes and different font sizes — real, reproducible numbers answering a different question. **Resize to a real desktop width (1440px is a safe default) before comparing computed CSS or screenshots against a desktop-designed prototype**, and re-take any measurement that looks wrong at a narrow width before reporting it as a defect — a common false positive worth ruling out first, not last. (The inverse check — deliberately testing the narrow width after touching the shell — is #30.)
